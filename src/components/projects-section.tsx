...
function ProjectCard({
  ...
  return (
    <div
      className={`w-full min-h-[350px] mb-20 flex overflow-x-hidden ${shouldInvert ? 'flex-row-reverse' : ''}`}
    >
      <div className="w-[55%] grow shrink-0 overflow-hidden rounded-lg h-[350px]">
        <div className="w-full h-full bg-primary-text-500"></div>
      </div>
      <div
        className={`w-[45%] grow shrink-0 ${shouldInvert ? 'text-left' : 'text-right'} relative h-max flex flex-col justify-center items-end ${shouldInvert ? 'items-start' : ''}`}
      >
        ...
        <div className="mt-8 relative p-4 h-max">
          ...
          <div
            className={`absolute z-0 top-0 w-[500px] h-full bg-background-200 rounded-lg ${shouldInvert ? 'left-0' : 'right-0'}`}
          ></div>
        </div>
        <div
